...
int main()
{
...

    {
        ...
        /*
        Using debugger, we can see that all of them are initialized with 
        zeros.
        The compiler provided a default constructor and default destructor.
        Here, this code initializes a class with the values of zero, then
        destroys it.
        In general case, the class is filled with default values and
        destroyed.
        */
    }
    ...
    
    {
        ...
        /*
        An error occurred because when we create a custom constructor,
        the compiler no longer creates the default one, so we need to
        handle all the process.
        After we create a default constructor on our own, it works correctly.
        */
    }
...

    {
        ...
        /*
        Here, the called constructors are:
        1) Parameterized for rect1 (by programer)
        2) Copy for rect2 (by compiler)
        3) Copy for rect3 (by compiler)
        */
    }
    ...
    /*
    Destructor is the function which destroys an object of a class.
    It is called when quitting the space where the class object is declared.
    In the very beginning, my class has no initialized destructor.
    In my case, the compiler creates a default destructor to destroy an object.
    In general case, destructors destroy an object and free up memory.
    So far, destructors were called 5 times.
    */
    ...
    {
        std::cout << "Task 1.6" << std::endl;
        Rect r1; // Def
        Rect *pR = new Rect(1,2,1,2); // Param   
        {
            Rect r2(r1); // Copy
            Rect arRect[2]; // Def x2
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // Param
                Rect r4(*pR); // Copy
                Rect r5(i,i,i,i); // Param
                // two Dest (r4, r5)
            } // Dest (r3)
        } // Dest (arRect x2, r2)
        delete pR; // Dest (obj which pR points at)
        // Dest r1
    }
...

    {
        ...
        /*
        Since the var is set to private, it cannot be called this way.
        */
    }
     ...
     */
     /*
1  **Execution Context**:
   - **Member Function**: Executes in the context of an object of the class and has access to its members.
   - **Normal Function**: Executes in the global context or within the context of a module.

2. **Access to Class Members**:
   - **Member Function**: Has access to the variables and methods of the class.
   - **Normal Function**: Does not have access to class members unless they are passed as parameters.

3. **Access Specifiers**:
   - **Member Function**: Can have access specifiers (`public`, `protected`, `private`).
   - **Normal Function**: Does not have access specifiers.

4. **Use of `this` Pointer**:
   - **Member Function**: Can use the `this` pointer to access class members.
   - **Normal Function**: Does not have a `this` pointer.
   */
    ...
    {
        Rect r1(1, 2, 3, 4);
        std::cout << r1.getWidth() << " " << r1.getHeight() << " " << r1.getSquare() << std::endl;
        r1.setWidth(10);
        r1.setHeight(10);
        std::cout << r1.getWidth() << " " << r1.getHeight() << " " << r1.getSquare() << std::endl;
    }
     ...
     */
     ...
     {
        int n = 10; // Например, вычислим 10-е число Фибоначчи
        Matrix fib_matrix = fibonacci_matrix(n);

        std::cout << "Fibonacci number F(" << n << ") = " << fib_matrix.get(0, 0) << std::endl;
    }
...

    {
        ...
        str2.print(); // Вывод строки "Hello, World!"

        str2.set(0, 'h');
        str2.print(); // Вывод строки "hello, World!"

        str2.set_new_string("New String");
        str2.print(); // Вывод строки "New String"

        std::cout << "Enter a new string: ";
        str2.read_line();
        str2.print(); // Вывод строки, введенной пользователем
    }
    ...
    // TODO☑: это надо переделать... Суть в чём: мы задаём всё ещё углы. Ширина и высота только вычисляется.
    // TODO☑: обозвать все параметры во всех методах / функциях.
    
    /*
    I had to change all the member-functions but those which only contain left_ 
    and bottom_ fields (e.g. getLeft()).

    As it was mentioned in the task, no changes in non-member-functions were made.
    */
